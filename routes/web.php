...
Route::get('/', function(){
    ...
    // Get ALL featured products (no limit per category)
    ...
    // Get products by category (for individual sections)
    $categorySlugs = ['cakes', 'traditional-sweets', 'cookies-biscuits', 'pastries-savories', 'breads', 'buns-rolls', 'dairy-products'];
    $categoryProducts = [];
...
Route::get('/contact', function() {
    // Load CMS sections for contact page
    $cmsSections = \App\Models\CmsSection::where('cms_page_id', 7)
        ->where('is_active', true)
        ->orderBy('sort_order')
        ->get()
        ->keyBy('section_key');
...
Route::get('/return', function() {
    // Load CMS sections for Return Policy page
    $cmsSections = \App\Models\CmsSection::where('cms_page_id', 4)
        ->where('is_active', true)
        ->orderBy('sort_order')
        ->get()
        ->keyBy('section_key');
