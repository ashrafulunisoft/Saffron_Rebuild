...
@section('title', "Edit {$coupon->code} - Saffron Admin")
...
<div class="role-container" style="max-width: 1000px;">
    <div class="glass-card glass-card-dark">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 3.5rem; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 1.5rem;">
            ...
            <h2 class="fw-800 mb-0 text-white letter-spacing-1" style="font-size: 2rem;">Edit Coupon</h2>
        </div>
        ...
        <form action="{{ route('admin.ecommerce.coupons.update', $coupon) }}" method="POST">
                ...
                <div class="col-md-6">
                    ...
                    <div class="position-relative">
                        <select name="type" class="input-dark input-custom" required>
                            <option value="percent" @if(old('type', $coupon->type) === 'percent') selected @endif>Percentage (%)</option>
                            <option value="fixed" @if(old('type', $coupon->type) === 'fixed') selected @endif>Fixed Amount (৳)</option>
                        </select>
                    </div>
                    @error('type')
                        <div class="text-danger mt-2" style="font-size: 0.8rem;">{{ $message }}</div>
            ...
            <div class="permission-title">Current Statistics</div>
            <div class="row g-4 mb-5">
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    <div style="background: rgba(15, 23, 42, 0.4); border: 1px solid rgba(59, 130, 246, 0.2); border-radius: 12px; padding: 1rem;">
                        <label class="text-muted" style="font-size: 0.75rem;">TIMES USED</label>
                        <div><span class="badge badge-visit-type">{{ $coupon->usage_count }}</span></div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div style="background: rgba(15, 23, 42, 0.4); border: 1px solid rgba(59, 130, 246, 0.2); border-radius: 12px; padding: 1rem;">
                        <label class="text-muted" style="font-size: 0.75rem;">CREATED</label>
                        <div class="text-white" style="font-size: 0.9rem;">{{ $coupon->created_at->format('M d, Y') }}</div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div style="background: rgba(15, 23, 42, 0.4); border: 1px solid rgba(59, 130, 246, 0.2); border-radius: 12px; padding: 1rem;">
                        <label class="text-muted" style="font-size: 0.75rem;">ORDERS</label>
                        <div><span class="badge badge-approved">{{ $coupon->orders()->count() }}</span></div>
                    </div>
                </div>
            </div>
            ...
            @if($coupon->orders()->count() > 0)
                <div class="p-3 rounded mb-5" style="background: rgba(251, 191, 36, 0.1); border: 1px solid rgba(251, 191, 36, 0.3);">
                    ...
                    <span style="font-size: 0.85rem; opacity: 0.8;">
                        This coupon has been used in {{ $coupon->orders()->count() }} order(s). Changes will affect future uses only.
                    </span>
                </div>
            @endif
